...
#
# With --mock, point the backend at the mock by setting in .env:
...
if [ "$WITH_MOCK" = "1" ]; then
  echo "[dev] Mock Phoenix ERP → http://localhost:9000  (set PHOENIX_API_BASE_URL=http://localhost:9000)"
  (cd "$BACKEND" && exec "$VENV_PY" -m uvicorn app.mock.phoenix:app --host 0.0.0.0 --port 9000) &
  PIDS+=($!)
fi
